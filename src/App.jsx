...
import { ShoppingBag, Play, Pause, X, ChevronRight, DoorOpen, Loader2, Disc } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-black text-white font-sans selection:bg-yellow-500 selection:text-black">
      ...
      {/* Full viewport cover environment background */}
      <div 
      ...
      <nav className="absolute top-0 left-0 w-full p-6 flex justify-between items-center z-40">
        ...
        {/* Customized main logo header: displays only MAYÉ */}
        <div className="absolute left-1/2 -translate-x-1/2 text-2xl md:text-3xl font-serif tracking-widest font-bold whitespace-nowrap drop-shadow-md select-none">
          MAYÉ
        </div>
        <button className="flex items-center gap-2 hover:text-yellow-400 transition-colors text-sm font-medium tracking-widest uppercase bg-black/25 backdrop-blur-md px-4 py-2 rounded-full border border-white/10">
          <ShoppingBag size={18} />
        ...
        {activeProduct && (
          <>
            ...
            <div className="flex-1 mt-12 overflow-y-auto pr-2">
              ...
              {/* Clothes Sizes selection */}
              {!['h1', 'h2', 'p6', 'p7', 'p8'].includes(activeProduct.id) && (
                <div className="space-y-3 mb-8">
                  <label className="text-xs tracking-widest uppercase text-gray-400">Select Size</label>
                  <div className="flex gap-2">
                    {['S', 'M', 'L', 'XL'].map(size => (
                      <button 
                        key={size} 
                        onClick={() => setSelectedSize(size)}
                        className={`w-10 h-10 rounded border flex items-center justify-center text-xs font-bold transition-all ${
                          selectedSize === size ? 'border-yellow-500 text-yellow-500 bg-yellow-500/10' : 'border-white/20 text-white hover:border-white'
                        }`}
                      >
                        {size}
                      </button>
      ...
      {}
      <div className="absolute bottom-6 left-6 right-6 flex flex-col md:flex-row justify-between items-center gap-4 z-40 pointer-events-none">
        
        {/* Suga & Spice Music Player positioned inside the bottom-left footer slot */}
        <div className="flex items-center gap-4 bg-[#141414]/90 backdrop-blur-md py-3 px-6 rounded-full border border-white/15 shadow-2xl transition-all duration-300 hover:border-yellow-500/30 pointer-events-auto self-start md:self-auto w-full max-w-[280px]">
          <button 
          ...
          {isPlaying ? (
            ...
            <Disc size={14} className="text-gray-500 ml-auto" />
          )}
        </div>

        {}
        {/* Scene Switcher Capsule */}
        <div className="flex bg-[#141414]/85 backdrop-blur-md rounded-full border border-white/15 p-1 w-full sm:w-auto shadow-2xl pointer-events-auto self-end md:self-auto">
